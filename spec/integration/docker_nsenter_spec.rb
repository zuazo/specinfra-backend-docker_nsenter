...
describe Specinfra::Backend::DockerNsenter do
  ...
  context '#docker_run!' do
    ...
    let(:final_cmd) do
      "sudo  -- nsenter -t #{pid} -m -u -i -n -p sh -c "\
      'ls\\ -la\\ /root/spa\\\\\\ ce'
    end
    before do
    ...
    it 'runs the nsenter command' do
    ...
    context 'with nsenter failures' do
      let(:stderr_str) { 'nsenter: failed to get the init pid' }
      let(:status) { 1 }

      it 'raises an exception' do
        expect { subject.send(:docker_run!, cmd) }.to raise_error(
          Specinfra::Backend::DockerNsenter::NsenterError, /^nsenter:/
        )
      end
    end
